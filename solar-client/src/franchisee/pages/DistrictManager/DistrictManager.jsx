...
const FranchiseDistrictManager = () => {
    ...
    const fetchManagers = useCallback(async () => {
        try {
            ...
            const res = await userAPI.getAll({ role: 'franchiseeManager' });
            ...
            try {
                const approvalRes = await getApprovals({ type: 'districtManager', status: 'Pending' });
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                        ...
                        <div>
                            ...
                            {(() => {
                                const matchedDistrict = districts.find(d => d._id === manager.district);
                                return matchedDistrict ? matchedDistrict.name : "Unknown District";
                            })()}
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="flex overflow-x-auto pb-3 mb-1 space-x-3">
                        ...
                        {districts.map((district) => (
                            <div
                                key={district._id}
                                className={`min-w-[160px] cursor-pointer rounded-lg border bg-white shadow-sm transition-all duration-300 ${selectedDistrict === district._id ? 'bg-blue-500 border-blue-500 text-white' : 'hover:border-blue-300'
                                    }`}
